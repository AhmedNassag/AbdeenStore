...
class ProductsController extends BaseController
{
    ...
    public function index(request $request)
    {
        try {
            ...
            ->when($request->machine_number != null,function ($q) use($request){
                return $q->where('machine_number','like','%'.$request->machine_number.'%');
            })
            ->when($request->motor_number != null,function ($q) use($request){
                return $q->where('motor_number','like','%'.$request->motor_number.'%');
            })
            ->when($request->color != null,function ($q) use($request){
                return $q->where('color','like','%'.$request->color.'%');
            })
            ->when($request->model != null,function ($q) use($request){
            ...
            foreach ($data as $product) {
                ...
                $item['machine_number']   = $product->machine_number;
                $item['motor_number']     = $product->motor_number;
    ...
    public function store(Request $request)
    {
        try {
            $this->validate($request, [
                'name'             => 'required',
                'machine_number'   => 'required|unique:products',
                'motor_number'     => 'required|unique:products',
                'color'            => 'required',
                'model'            => 'required',
                'type'             => 'required',
                'cost'             => 'required',
    ...
    public function update(Request $request)
    {
        try {
            $this->validate($request, [
                'name'             => 'required',
                'machine_number'   => 'required|unique:products,machine_number,'.$request->id,
                'motor_number'     => 'required|unique:products,motor_number,'.$request->id,
                'color'            => 'required',
                'model'            => 'required',
                'type'             => 'required',
                'cost'             => 'required',
