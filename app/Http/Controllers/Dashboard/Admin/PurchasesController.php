...
class PurchasesController extends BaseController
{
    ...
    {
        try {
            if($request->search_number != null)
            {
                $purchaseDetails = PurchaseDetail::with('product')
                ->whereRelation('product','machine_number',$request->search_number)
                ->OrWhereRelation('product','motor_number',$request->search_number)
                ->get();
                if($purchaseDetails->count() > 0)
                {   
                    $purchases = Purchase::with('facture', 'provider', 'warehouse')
                    ->where('id',$purchaseDetails[0]->purchase_id)
            ...
            $purchase_details = PurchaseDetail::get('product_id');
            foreach($purchase_details as $item)
            {
                $ids[] = $item->product_id;
            }
            $products   = product::whereNotIn('id', $ids)->where('deleted_at', '=', null)->get(['id', 'name', 'machine_number', 'motor_number']);
