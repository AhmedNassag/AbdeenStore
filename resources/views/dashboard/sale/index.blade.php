    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main.Sales') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
        ...
        <!-- notAvailableQuantity Notify -->
        @if (session()->has('notAvailableQuantity'))
            <script id="notAvailableQuantity">
                window.onload = function() {
                    notif({
                        msg: "الكمية المطلوبة غير متاحة فى المخزن حالياً..!",
                        type: "error"
                    })
                }
            </script>
        ...
        <div class="row">
                    ...
                    <div id="print">
                        <div class="card-header pb-0">
                            ...
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0">{{ trans('main.Sales') }}</h4>
                            ...
                            <div class="row row-xs">
                                <div class="col-sm-6 col-md-12 notPrint">
                                    ...
                                    @else
                                        @can('عرض المبيعات')
                                            <a class="btn btn-danger ripple float-left" href="{{ route('sales.index') }}">
                                    ...
                                    <form action="{{ route('sales.index') }}" method="get">
                                        <div class="row mt-5">
                                            <div class="col-3 mr-2 notPrint">
                                                <label for="search_number" class="">{{ trans('main.Search With Number') }} :</label>
                                                <input id="search_number" type="text" class="form-control" name="search_number" value="{{ $search_number }}">
                                            </div>
                                ...
                                <table class="table table-striped" id="example1" data-page-length='50' style="text-align: center;">      
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($sales as $item)
                                                ...
                                                <td class="text-center notPrint">
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu tx-13 bd-primary rounded-5">
                                                            @if($trashed == false)
                                                                @can('عرض المبيعات')
                                                                    <a class="dropdown-item" href="{{ route('sales.show',[$item->id]) }}" title="{{ trans('main.Show') }}">
                                                                        <i class="text-success fas fa-eye"></i>&nbsp;&nbsp;{{ trans('main.Show') }}
                                                                    </a>
                                                                @endcan
                                                                @can('عرض دفع المبيعات')
                                            ...
                                            @include('dashboard.sale.forceDeleteModal')
                                            @include('dashboard.sale.restoreModal')
                                            @include('dashboard.sale.addPaymentModal')
                                            @include('dashboard.sale.showPaymentsModal')
            ...
            @include('dashboard.sale.addModal')
            @include('dashboard.sale.deleteSelectedModal')
...
@section('js')
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            $.each(data,function(key,value) {
                                $('select[name="product_id[]"]').append('<option class="form-control" value="'+ value["product_id"] +'">' + value["product_name"] + ' (رقم الشاسيه: ' + value["product_machine_number"] + ') ' + ' (رقم الموتور: ' + value["product_motor_number"] + ') ' + '</option>');
                            });
                        }
                    });
                } else {
    ...
    <script type="text/javascript">
        function addRow()
        {
            $('table[id="myTable"]').append('<tr><td style="width:48%;"><label for="product_id" class="mr-sm-2">{{ trans('main.Product') }} :</label><select class="form-control select2" name="product_id[]" required><option label="{{ trans('main.Choose') }}"></option></select></td><td style="width:3%;"></td><td style="width:48%;"><label for="quantity" class="mr-sm-2">{{ trans('main.Quantity') }} :</label><input id="quantity" type="number" class="form-control" name="quantity[]" value="1" value="{{ old('quantity') }}" required></td><td style="width:1%;"></td></tr>');
        }
    </script>
    <script type="text/javascript">
        function removeRow()
